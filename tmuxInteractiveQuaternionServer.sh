...
tmux send-keys "sudo python quaternions.py localhost 10500" C-m
...
tmux new-window -t $SESSION:8 -n 'SIM_LOG'
tmux send-keys "tail -f ~/temp/_OPSSAT_SIMULATOR/Sim.log" C-m

tmux new-window -t $SESSION:9 -n 'TRIPLE_PRESENTATION'
tmux send-keys "cd ~/repos/TestBed_VM_Software/TriplePresentation" C-m
tmux send-keys "bash runDemo_RPI3.sh" C-m

tmux new-window -t $SESSION:10 -n 'DETACH'
